...
#    зберігаємо html сторінку локально
    with open('data/bookclub/children_books/main/bookclub.html', 'w') as file:
        file.write(req.text)

    # відкриваємо локальний файл html, щоб зробити меншу нагрузку на сайт
    with open('data/bookclub/children_books/main/bookclub.html') as file:
        src = file.read()
    ...
    for project_url in project_urls:                   # зробимо зріз на одну книгу
        ...
        with open(f'data/bookclub/children_books/data/{project_name}.html', 'w') as file:
            file.write(req.text)

        with open(f'data/bookclub/children_books/data/{project_name}.html') as file:
            src = file.read()
    ...
    with open('data/bookclub/children_books/main/bookclub_children_books.json', 'a', encoding='utf-8') as file:
        json.dump(book_data_list, file, indent=4, ensure_ascii=False)
    print('Done!')
         
base_url = 'https://bookclub.ua/catalog/books/dityacha-literatura/?i='
listmode = '&listmode=2'
start_value = 0
step = 20
max_attempts = 21  # максимальна кількість ітерацій
