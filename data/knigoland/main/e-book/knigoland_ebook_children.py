...
def get_data(url):
    ...
    with open('data/knigoland/main/edu_books/knigoland_ebook_children.html', 'w') as file:
        file.write(req.text)

    # відкриваємо локально
    with open('data/knigoland/main/edu_books/knigoland_ebook_children.html') as file:
        src = file.read()
    ...
    with open('data/knigoland/data/processed/json/knigoland_ebook_children.json', 'a', encoding='utf-8') as file:
...
print(f'Page: 1')
get_data('https://knigoland.com.ua/dityacha-literatura-elektronni-knigi') # ТІЛЬКІ ОДНА СТОРІНКА

get_data('https://knigoland.com.ua/pidlitkova-literatura--') # СЮДИ Ж ЗБЕРІГАЮ ПІДЛІТКОВУ ЛІТЕРАТУРУ

# робимо цикл щоб пройтись по всім сторінкам
# base_url = 'https://knigoland.com.ua/Non-fiction-ebook?PAGEN_1=' 

# # встановлюємо кількість циклів всього 1 сторінок

# for url_num in range(2, 4):    # треба ставити range максимальний на одну сторінку більше
#     try:
